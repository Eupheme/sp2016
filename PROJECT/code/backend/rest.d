...
class APIImpl : IAPI
{
	...
	string postLogin(string username, string password)
	{
		...
		auto rows = db.query("SELECT user_id, username, status, first_name, last_name, email, birth_date, added_date, picture FROM users WHERE username = ? AND CAST(password AS CHAR(40)) LIKE CONCAT('%', ?, '%') AND status < 2", username, pass);
